  ...
  return (
    <div className="d-flex flex-column vh-100 position-fixed" 
      ...
      <nav className="nav flex-column gap-1 px-3">
        ...
        <div className="mb-2">
          <button 
            className={`btn btn-link text-start w-100 d-flex align-items-center justify-content-between gap-2 py-2 px-3 rounded ${
              isActive('/listpatient') ? 'bg-sky-50 text-sky-700' : 'text-sky-600'
            }`}
          ...
          <div className={`collapse ${isActive('/listpatient') ? 'show' : ''}`} id="patientMenu">
            <nav className="nav flex-column ms-4">
              <Link 
                className={`nav-link py-2 ps-3 rounded ${
                  isActive('/listpatient') ? 'bg-sky-50 text-sky-700 border-start border-sky-500' : 'text-sky-600'
                }`}
                to="/listpatient"
              >
        ...
        <Link 
          className={`nav-link d-flex align-items-center gap-2 py-2 px-3 rounded ${
            isActive('/CreatePatient') ? 'bg-sky-100 text-sky-800 fw-medium' : 'text-sky-600'
          }`}
          to="/CreatePatient"
        >
        ...
        <Link 
          className={`nav-link d-flex align-items-center gap-2 py-2 px-3 rounded ${
            isActive('/Listappointment') ? 'bg-sky-100 text-sky-800 fw-medium' : 'text-sky-600'
          }`}
          to="/Listappointment"
        >
          <i className="bi bi-person-plus" style={{ width: '24px', fontSize: '1.1rem' }}></i>
        ...
        <div className="mt-4 mb-2 text-uppercase small fw-bold text-sky-600 ps-3">
         ...
         <Link 
          className={`nav-link d-flex align-items-center gap-2 py-2 px-3 rounded ${
            isActive('/Appointment') ? 'bg-sky-100 text-sky-800 fw-medium' : 'text-sky-600'
          }`}
